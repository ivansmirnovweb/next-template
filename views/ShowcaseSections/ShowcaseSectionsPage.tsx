    ...
    <div className="layout-container py-8 sm:py-10">
      <div className="space-y-8">
        ...
        <div className="space-y-10">
          ...
          <ShowcaseSection
            title="About"
            description="About-section patterns for a concise introduction, an origin story, and the people responsible for the work."
          >
            <ul className="grid gap-4">
              {showcaseSectionEntriesByGroup.about.map((entry) => (
                <ShowcaseNavItem
                  key={entry.slug}
                  title={entry.title}
                  description={entry.description}
          ...
          <ShowcaseSection
            title="Services"
            description="Service-discovery patterns for broad browsing, needs-based guidance, and a focused commercial offer."
          >
            <ul className="grid gap-4">
              {showcaseSectionEntriesByGroup.services.map((entry) => (
                <ShowcaseNavItem
                  key={entry.slug}
                  title={entry.title}
                  description={entry.description}
          ...
          >
            <ul className="grid gap-4">
              {showcaseSectionEntriesByGroup.process.map((entry) => (
                <ShowcaseNavItem
                  key={entry.slug}
                  title={entry.title}
                  description={entry.description}
          ...
          <ShowcaseSection
            title="Contacts"
            description="Contact-section patterns for choosing a channel, finding a location, and reaching the responsible team."
          >
            <ul className="grid gap-4">
              {showcaseSectionEntriesByGroup.contacts.map((entry) => (
                <ShowcaseNavItem
                  key={entry.slug}
                  title={entry.title}
                  description={entry.description}
          ...
          >
            <ul className="grid gap-4">
              {showcaseSectionEntriesByGroup.cta.map((entry) => (
                <ShowcaseNavItem
                  key={entry.slug}
                  title={entry.title}
                  description={entry.description}
